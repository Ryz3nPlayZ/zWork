"""Tests for MCP config loading and tool-name encoding."""
import asyncio
import json
import unittest
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_enabled_false_is_kept_in_list(self) -> None:
        path = self._write({"mcpServers": {"off": {"command": "echo", "args": ["hi"], "enabled": False}}})
        specs = load_config(path)
        self.assertEqual(len(specs), 1)
    ...
    def test_skips_invalid_rows(self) -> None:
        ...
        specs = load_config(path)
        self.assertEqual(sorted(s.name for s in specs), ["good"])
